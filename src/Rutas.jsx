...
function Rutas() {
  ...
  useEffect(() => {
    ...
    const getUserRol = async () => {
      const docSnapshot = await getDoc(doc(firebaseExports.db, "users", userLogged));
  ...
  return (
    <Routes>
      ...
      <Route exact path="/" element={<Stores comercios={comercios} setIdComercio={setIdComercio} userRol={userRol} />} />
      ...
      <Route exact path="/:comercio/shop" element={
        <Home
        ...
        />} 
      />
      <Route exact path="/:comercio/admin" element={
        <AdminView
          setProductos={setProductos}
          productos={productos}
          idComercio={idComercio}
