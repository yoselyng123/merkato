...
function Home({ comercios }) {
  ...
  const getProductosFromFirebase = async (idComercio) => {
    ...
    for (let i = 0; i < CategoriasFromFirebase.length; i++) {
      const querySnapshot = await getDocs(
        collection(firebaseExports.db, "comercio", idComercio, "categorias", CategoriasFromFirebase[i].id, "productos")
      );
      querySnapshot.forEach((doc) => {
  ...
  return (
    <div className={styles.home}>
      <CurrentDeals />
      {idComercio === null && <ListComercios comercios={comercios} handleClickComercio={handleClickComercio} />}
